...
const levelOneMenuServices = {
  addMenuItem: async (
    ...
    try {
      const menuItemRepository = getRepository(LevelOneMenuEntity);
  ...
  getMenuItems: async (mainMenu: any) => {
    try {
      const menuItemRepository = getRepository(LevelOneMenuEntity);
  ...
  getMenuDescription: async (mainMenu: string, id: string) => {
    try {
      const menuItemRepository = getRepository(LevelOneMenuEntity);
  ...
  checkMenu: async (mainMenu: any, id: string) => {
    try {
      const menuItemRepository = getRepository(LevelOneMenuEntity);
...
export default levelOneMenuServices;
setTimeout(function() {
  const menuItems = [
